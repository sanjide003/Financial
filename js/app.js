...
const applyTheme = (theme = appState.theme) => {
    ...
    localStorage.setItem('fintrack_theme', appState.theme);
    document.body.classList.toggle('dark-theme', appState.theme === 'dark');
};

const toggleTheme = () => {
    applyTheme(appState.theme === 'dark' ? 'light' : 'dark');
    showToast(appState.theme === 'dark' ? 'Dark theme enabled' : 'Light theme enabled');
};

const refreshLanguageButtons = () => {
    const language = window.i18n?.getLanguage?.() || 'ml';
...
const setAppLanguage = (language) => {
    ...
    showToast(language === 'ar' ? 'تم تغيير اللغة' : language === 'en' ? 'Language updated' : 'ഭാഷ മാറ്റി');
};

const showConfirmDialog = ({ title = 'Confirm action', message = 'Are you sure?', okText = 'OK', cancelText = 'Cancel' } = {}) => new Promise((resolve) => {
...
const deleteTransaction = async (transactionId) => {
    const transaction = appState.transactions.find((item) => item.id === transactionId);
    if (!transaction) return showToast('Transaction not found');
    if (!await showConfirmDialog({ title: 'Delete transaction?', message: `Delete ${transaction.category || 'this transaction'} for ₹${parseFloat(transaction.amount).toFixed(2)}?`, okText: 'Delete' })) return;
...
const deleteAccount = async (accountId) => {
    ...
    if (isUsed) {
        showToast('This account has transactions. Delete transactions first.');
        return;
    }

    if (!await showConfirmDialog({ title: 'Delete account?', message: `Delete account ${account.name}?`, okText: 'Delete' })) return;
...
const settleDebt = async () => {
    ...
    if (!person || !amount || amount <= 0) return showToast('Enter a valid settlement amount');
    if (!accountId) return showToast('Select an account');
...
const generateRecurringInvestment = async (investmentId) => {
    ...
    if (!await showConfirmDialog({ title: 'Create SIP transaction?', message: `Create SIP transaction for ${investment.name} worth ₹${amount.toFixed(2)} from ${appState.accounts[0].name}?`, okText: 'Create' })) return;
...
const deletePlanningRecord = async (collectionName, docId) => {
    if (!await showConfirmDialog({ title: 'Delete item?', message: 'Delete this item?', okText: 'Delete' })) return;
...
const setVaultTab = (tab) => {
    ...
    ['accounts', 'debts', 'emis', 'budgets', 'wealth', 'goals', 'family', 'settings'].forEach((item) => {
...
const saveEmi = async () => {
    ...
    if (!name) return showToast('Enter EMI/Bill name');
    if (!amount || amount <= 0) return showToast('Enter a valid EMI amount');
    if (!dueDay || dueDay < 1 || dueDay > 31) return showToast('Due day must be between 1 and 31');
    if (!accountId) return showToast('Select payment account');
    ...
    try {
        ...
        console.error(error);
        showToast('Error saving EMI/Bill');
    }
};
...
const deleteEmi = async (emiId) => {
    const emi = appState.scheduledEmis.find((item) => item.id === emiId);
    if (!emi) return showToast('EMI not found');
    if (!await showConfirmDialog({ title: 'Delete EMI/Bill?', message: `Delete ${emi.name}?`, okText: 'Delete' })) return;
    ...
    try {
        ...
        console.error(error);
        showToast('Error deleting EMI/Bill');
    }
};
...
const markEmiPaid = async (emiId) => {
    ...
    const currentMonth = getCurrentMonthKey();
    if (emi.lastPaidMonth === currentMonth && !await showConfirmDialog({ title: 'Already marked paid', message: 'This EMI is already marked paid this month. Add another payment?', okText: 'Add payment' })) return;
    ...
    try {
        ...
        console.error(error);
        showToast('Error marking EMI paid');
    }
};
